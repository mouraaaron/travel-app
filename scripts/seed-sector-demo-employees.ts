...
// Seed pontual para a apresentação: adiciona 5 funcionários mock com setor
// fixo cada (em vez do setor aleatório usado por seed-demo-data.ts) mais
// suas viagens (feitas e em aberto), para popular os gráficos por setor no
// Painel do admin. Não cria acesso de login relevante para a demo — os
// usuários existem no auth apenas porque profiles.id referencia auth.users,
// mas ninguém precisa logar com eles.
...
// Distribuição pedida: 1 marketing, 2 produto, 1 founder, 1 engenharia.
const NEW_EMPLOYEE_SECTORS: Sector[] = ["marketing", "product", "product", "founders", "engineering"];
...
// Maioria confirmed/approved (viagens feitas), com pending_admin (em aberto)
// e alguns rejected/cancelled — mistura realista, igual seed-demo-data.ts.
...
async function createEmployee(
  ...
  sector: Sector
): Promise<{ id: string; fullName: string; sector: Sector }> {
...
async function main() {
  ...
  const newEmployees = await Promise.all(NEW_EMPLOYEE_SECTORS.map((sector) => createEmployee(org.id, sector)));

  const requests = newEmployees.flatMap(({ id, sector }) =>
